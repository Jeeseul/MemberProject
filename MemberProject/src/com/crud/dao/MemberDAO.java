...
public class MemberDAO {
	...
	private final String M_INSERT = "insert into member(userid, password, username, email, photo, detail,sid) values (?,sha1(?),?,?,?,?,?)";
	...
	public MemberVO getMember(int sid) {
		...
		try {
			...
			if(rs.next()) {
				...
				System.out.println(one.getUserid()+"-"+one.getUsername()+"-"+one.getEmail() +"-"+ one.getPhoto()+"-"+ one.getDetail()+"-"+one.getSid());	
			}
			rs.close();
		} catch (Exception e) {
			e.printStackTrace();
